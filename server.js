...
// --- TEMP: seed/reset admin (DELETE AFTER USE) ---
app.post('/internal/seed-admin', async (req, res) => {
  const secretHeader = (req.headers['x-seed-secret'] || '').toString();
  if (secretHeader !== MIGRATE_SECRET) {
    return res.status(401).json({ error: 'unauthorized' });
  }

  try {
    const email = 'aluminiportalddvscm@gmail.com';
    const password = 'ddvsc@123';
    const hash = await bcrypt.hash(password, 10);
    ...
    return res.json({ ok: true, message: 'Admin seeded/reset successfully', email });
  } catch (err) {
    console.error('Seed admin error:', err);
    return res.status(500).json({ ok: false, error: 'failed to seed admin', detail: String(err) });
  }
});

// --- Seed admin user if missing (keeps original auto-seed for safety) ---
(async () => {
  try {
    const adminEmail = 'aluminiportalddvscm@gmail.com';
    const adminPassword = 'ddvsc@123';
